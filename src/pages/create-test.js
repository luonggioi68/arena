...
import { 
    ...
    Upload, X, Flame, AlertTriangle, Home // Bổ sung AlertTriangle, Home
} from 'lucide-react';
...
1. Phần I. Trắc nghiệm nhiều lựa chọn: Tổng ${getTotalQ_Normal('mcq')} câu (Biết: ${matrix.mcq.b}, Hiểu: ${matrix.mcq.h}, Vận dụng: ${matrix.mcq.vd}).
    ...
    - Đáp án không được trùng lặphương án đúng, phải có phương án sai lạc hợp lý để phân loại học sinh. Không được tất cả đều đúng, A và B đúng hoặc Sai,....
  ...
  if (isTeacher === false) {
      return (
        <div className="min-h-screen bg-[#050505] flex items-center justify-center flex-col text-center p-4 relative overflow-hidden selection:bg-red-500 selection:text-white">
          ...
          <p className="text-slate-400 mb-10 font-bold tracking-widest text-xs md:text-sm relative z-10 px-4 max-w-lg">Công cụ Soạn Đề AI là vũ khí tuyệt mật. Bạn cần đăng nhập bằng tài khoản <span className="text-red-400 font-black">GIÁO VIÊN</span> để sử dụng.</p>
