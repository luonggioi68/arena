...
- TOÁN HỌC/LÝ/HÓA: Công thức BẮT BUỘC dùng LaTeX kẹp trong $...$. Không dùng text thô.
- KHOẢNG TRẮNG CÔNG THỨC: BẮT BUỘC gõ thêm dấu cách (space) TRƯỚC và SAU mỗi công thức $...$.
...
**QUY TẮC ĐIỀN DỮ LIỆU ĐẶC TẢ (CỰC KỲ QUAN TRỌNG):**
1. Ở cột "Yêu cầu cần đạt", BẮT BUỘC dùng thẻ <br> để chia rõ 3 mức độ:
   + Biết: [...] <br>
   + Hiểu: [...] <br>
   + Vận dụng: [...]
2. Ở các cột điền Số câu hỏi, BẮT BUỘC ghi kèm Mã năng lực đặc thù bên dưới con số (dùng thẻ <br>). 
   *LƯU Ý QUAN TRỌNG CHO MÔN ${subject}: ${competencyGuidance}*
...
Tạo các bảng đáp án cho Phần I, Phần II và hướng dẫn chấm chi tiết cho Tự luận/Trả lời ngắn.
        `;

        const result = await model.generateContent(prompt);
        let textResult = result.response.text();

        // 1. Cắt râu ria (Tuyệt đối không lấy chữ gì trước PHẦN 1)
        ...
        // 2. Dọn rác Markdown bao quanh bảng HTML
        textResult = textResult.replace(/```html\n?/g, '');
        textResult = textResult.replace(/```\n?/g, '');

        // 3. LỚP ÁO GIÁP BẢO VỆ CÂU HƯỚNG DẪN TRƯỚC KHI ÉP XUỐNG DÒNG
        textResult = textResult.replace(/a\),\s*b\),\s*c\),\s*d\)/g, '@@ABCD_LOWER@@');

        // 4. THUẬT TOÁN ÉP XUỐNG DÒNG CÁC ĐÁP ÁN
        textResult = textResult.replace(/([^\n])\s+(\*?[A-D]\.)/g, '$1\n\n$2');
        textResult = textResult.replace(/([^\n])\s+(\*?[a-d]\))/g, '$1\n\n$2');
        textResult = textResult.replace(/(\n)(\*?[A-D]\.)/g, '\n\n$2');
        textResult = textResult.replace(/(\n)(\*?[a-d]\))/g, '\n\n$2');

        // 5. PHỤC HỒI CÂU HƯỚNG DẪN TỪ LỚP ÁO GIÁP
        textResult = textResult.replace(/@@ABCD_LOWER@@/g, 'a), b), c), d)');

        // 6. BẢO VỆ DẤU * CHO ĐÁP ÁN
        textResult = textResult.replace(/\n\n\*([A-D]\.)/g, '\n\n\\*$1');
        textResult = textResult.replace(/\n\n\*([a-d]\))/g, '\n\n\\*$1');

        // 7. KHỬ KHOẢNG TRẮNG CÔNG THỨC TOÁN HỌC (CHỐNG DÍNH CHỮ)
        textResult = textResult.replace(/\$([^$\n]+?)\$([\p{L}\p{N}])/gu, '$$$1$$ $2');
      ...
      <header className="h-[70px] bg-[#1e293b]/90 backdrop-blur border-b border-white/10 px-6 flex justify-between items-center shrink-0">
         <div className="flex items-center gap-4">
             ...
             <div>
                ...
                <p className="text-[10px] font-bold text-slate-400 uppercase">Khử Dính Chữ Toán Học & Tối ưu Giao Diện</p>
             </div>
         </div>
         {resultMarkdown && (
          ...
          <div className="w-full lg:w-[460px] bg-[#020617] border-r border-white/10 overflow-y-auto p-4 custom-scrollbar shrink-0">
              ...
              <div className="space-y-4">
                  ...
                  <div className="bg-slate-900/50 p-3 rounded-lg border border-red-500/30">
                      <label className="block text-[10px] font-black text-red-400 mb-1 uppercase">Phạm vi kiến thức</label>
                      <textarea 
                          value={testScope} 
                  ...
                  <button onClick={handleGenerateTest} disabled={loading} className="w-full bg-gradient-to-r from-red-600 to-orange-600 hover:from-red-500 hover:to-orange-500 text-white py-3 rounded-xl font-black uppercase tracking-widest shadow-lg shadow-red-500/20 active:scale-95 transition flex items-center justify-center gap-2 mt-2">
                      {loading ? <Loader2 className="animate-spin"/> : <Sparkles size={20}/>} {loading ? 'ĐANG TẠO BỘ ĐỀ...' : 'TẠO BỘ ĐỀ (NGANG)'}
                  </button>
              </div>
          </div>
          ...
          <div className="flex-1 bg-slate-300 overflow-y-auto relative p-4 md:p-8 custom-scrollbar">
              {loading ? (
                  <div className="h-full flex flex-col items-center justify-center text-slate-500">
                      ...
                      <p className="text-sm font-bold">Đang loại bỏ dính chữ và sửa lỗi định dạng...</p>
                  </div>
              ) : resultMarkdown ? (
                  <div 
                      ...
                      <div style={{ fontFamily: "'Times New Roman', serif" }}>
                          <ReactMarkdown 
                              ...
                              components={{
                                  ...
                                  td: ({node, ...props}) => <td style={{border: '1px solid black', padding: '6pt', verticalAlign: 'middle'}} {...props}/>,
                              }}
                          >
                              {resultMarkdown}
                          </ReactMarkdown>
