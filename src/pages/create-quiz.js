...
export default function CreateQuiz() {
  ...
  const fileInputRef = useRef(null);     // Ref cho upload Cơ bản
  const fileInputSTEMRef = useRef(null); // Ref cho upload AI (Mới)
  ...
  // --- HÀM UPLOAD ẢNH CHO MAMMOTH (Sửa lỗi is not defined) ---
  const processMammothImage = async (image) => {
    try {
        ...
        if (!res.ok) {
            ...
            // Không throw error để tránh dừng luồng đọc file, chỉ log lỗi
            return null;
        }
  ...
  // --- LOGIC GỐC CỦA BẠN (KHÔNG CHỈNH SỬA) ---
  ...
  const parseDocxContent = (htmlContent) => {
    ...
    elements.forEach(el => {
        ...
        if (qMatch) {
            ...
            // Xử lý ảnh trong câu hỏi nếu có
  ...
  const handleFileUpload = async (event) => {
    ...
    reader.onload = async (e) => {
      try {
        // Sử dụng processMammothImage để upload ảnh
        const options = {
            convertImage: mammoth.images.imgElement(processMammothImage)
        };
  ...
  // --- HÀM 2: UPLOAD AI (STEM - TOÁN/LÝ/HOÁ) ---
  const handleFileUploadSTEM = async (event) => {
    ...
    try {
        ...
        const prompt = `
        Bạn là chuyên gia xử lý dữ liệu đề thi Toán, Lý, Hóa. Hãy chuyển đổi HTML thô sau đây thành JSON.
        
        HTML ĐẦU VÀO: """${rawHtml}"""

        YÊU CẦU:
        1. **Toán:** Chuyển công thức thành LaTeX $...$.
        2. **Ảnh:** Giữ thẻ <img src="..."> và thay bằng " [img] ". Lấy link ảnh vào trường dữ liệu.
        3. **Phân loại:** [P1] MCQ (4 đáp án A-D), [P2] TF (4 ý a-d), [P3] SA (Key).

        OUTPUT JSON:
        [ 
          { "type": "MCQ", "part": 1, "q": "...", "img": "...", "a": ["A", "B", "C", "D"], "aImages": [], "correct": 0 },
        ...
        // --- BẮT ĐẦU SỬA LỖI BAD ESCAPED CHARACTER ---
        // Bước 1: Đưa tất cả backslash kép (nếu có) về dạng đơn để chuẩn hóa
        cleanJson = cleanJson.replace(/\\\\/g, '\\');
        // Bước 2: Nhân đôi TẤT CẢ các dấu backslash đơn để JSON.parse đọc được an toàn
        cleanJson = cleanJson.replace(/\\/g, '\\\\');
        // Bước 3: Khôi phục lại dấu nháy kép \" (vì bước 2 đã vô tình biến nó thành \\")
        cleanJson = cleanJson.replace(/\\\\"/g, '\\"');
        // --- KẾT THÚC SỬA LỖI ---
        ...
        const finalQuestions = parsedQuestions.map(q => ({
            ...
            type: q.type || 'MCQ',
            part: q.part || 1,
            q: (q.q || '').replace(/^(Câu|Bài)\s+\d+[:.]?\s*/i, '').trim(),
            img: q.img || '',
            a: Array.isArray(q.a) ? q.a : ['', '', '', ''],
            aImages: Array.isArray(q.aImages) ? q.aImages : ['', '', '', ''], 
    ...
    <div className="min-h-screen bg-slate-50 p-6 pb-32 font-sans text-slate-900">
      ...
      <header className="max-w-6xl mx-auto flex justify-between items-center mb-8 sticky top-0 bg-slate-50/95 backdrop-blur z-20 py-4 border-b border-slate-200">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleSave} disabled={loading || imgUploading} className="flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-bold shadow-lg transition">{loading || imgUploading ? <Loader2 className="animate-spin"/> : <><Save size={18} /> Lưu</>}</button>
        </div>
      </header>

      <main className="max-w-6xl mx-auto space-y-6">
        {/* ... (Giữ nguyên các phần UI: Thông tin chung, Cấu trúc điểm...) */}
