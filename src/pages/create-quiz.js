...
export default function CreateQuiz() {
  ...
  const handleGenerateAI = async () => {
    ...
    try {
      ...
      const prompt = `Đóng vai giáo viên môn ${aiSubject} lớp ${aiLevel}. Soạn đề thi chủ đề: "${aiTopic}". Tài liệu tham khảo: ${aiSource}. TUYỆT ĐỐI ĐÁP ÁN KHÔNG ĐƯỢC XUẤT RA CÂU TRẢ LỜI DẠNG TẤT CẢ ĐỀU ĐÚNG, A VÀ B ĐỀU ĐÚNG, HOẶC A VÀ B ĐỀU SAI. Mỗi câu hỏi phải có một đáp án đúng duy nhất. CẤU TRÚC ĐỀ THI: - PHẦN 1 (Trắc nghiệm): Tổng ${countTN} câu (${matrix.tn_biet} Biết, ${matrix.tn_hieu} Hiểu, ${matrix.tn_vd} Vận dụng). - PHẦN 2 (Đúng/Sai): Tổng ${matrix.ds_count} câu lớn. Mỗi câu BẮT BUỘC CÓ 4 ý con (${matrix.ds_biet} Biết, ${matrix.ds_hieu} Hiểu, ${matrix.ds_vd} Vận dụng). - PHẦN 3 (Trả lời ngắn): Tổng ${countTL} câu (${matrix.tl_biet} Biết, ${matrix.tl_hieu} Hiểu, ${matrix.tl_vd} Vận dụng). YÊU CẦU BẮT BUỘC: 1. Tuyệt đối KHÔNG chào hỏi, KHÔNG giải thích thêm. Chỉ in ra chuỗi JSON. 2. Công thức Toán phải dùng LaTeX kẹp trong $...$. 3. CHỈ dùng dấu $...$ cho công thức phức tạp như: mũ, căn bậc hai,logaric, công thức hoá học, vật lý (ví dụ: $\\frac{1}{2}$, $\\sqrt{x}$,). Công thức đơn giản như số, biến, biểu thức đơn giản,... (ví dụ: x > 0, 2x+2=0,..) KHÔNG cần dấu $. 4. Ký tự backslash (\\) trong LaTeX phải được escape thành (\\\\). (Ví dụ: "\\\\frac" thay vì "\\frac"). 5. ĐỐI VỚI CÂU ĐÚNG/SAI (TF), BẮT BUỘC PHẢI CÓ MẢNG "items" chứa 4 ý. OUTPUT CHỈ LÀ 1 MẢNG JSON CÓ CẤU TRÚC NHƯ SAU: [ { "type": "MCQ", "part": 1, "q": "...", "a": ["A", "B", "C", "D"], "correct": 0 }, { "type": "TF", "part": 2, "q": "...", "items": [ { "text": "...", "isTrue": true }, { "text": "...", "isTrue": false }, { "text": "...", "isTrue": true }, { "text": "...", "isTrue": false } ] }, { "type": "SA", "part": 3, "q": "...", "correct": "..." } ]`;
