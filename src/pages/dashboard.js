...
export default function Dashboard() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      // Tải dữ liệu các Tab
      await Promise.all([ 
          fetchQuizzes(currentUser), // Đã sửa: Truyền toàn bộ object user để check Master
  ...
  const fetchUserStats = async () => {
      try {
          ...
          setUserStats({
              ...
              activeTeachers: tActive,
              expiredTeachers: tExpired,
              totalStudents: sTotal
          });
      } catch (e) {
          console.error("Lỗi fetch user stats:", e);
      }
  };
  ...
  // --- HÀM TẢI ĐỀ THI (ĐÃ CẤP QUYỀN CHO MASTER XEM TOÀN BỘ KHO GAME) ---
  const fetchQuizzes = async (currUser) => { 
      try { 
          const isMaster = MASTER_EMAILS.includes(currUser.email);
          
          // 1. Luôn lấy đề của chính mình tạo
          const qOwn = query(collection(firestore, "quizzes"), where("authorId", "==", currUser.uid)); 
          const sOwn = await getDocs(qOwn); 
          let list = sOwn.docs.map(d => ({ id: d.id, ...d.data() })); 

          // 2. Nếu là Master thì lấy THÊM toàn bộ đề đang được Public trong GAME_REPO
          if (isMaster) {
              const qRepo = query(collection(firestore, "quizzes"), where("origin", "==", "GAME_REPO"));
              const sRepo = await getDocs(qRepo);
              const repoList = sRepo.docs.map(d => ({ id: d.id, ...d.data() }));
              
              // Gộp chung và loại bỏ các đề bị trùng lặp ID 
              const combined = [...list, ...repoList];
          ...
          // Cấp mã Code ngẫu nhiên cho những đề chưa có
          list.forEach(async (q) => { 
              if (!q.code) { 
                  const c = Math.floor(1000 + Math.random() * 9000).toString(); 
                  await updateDoc(doc(firestore, "quizzes", q.id), { code: c }); 
                  q.code = c; 
              } 
          }); 
          ...
          console.error("Lỗi fetchQuizzes:", e);
      } 
  };
 // Đưa user.uid vào tham số và filter trực tiếp từ Firebase
// Khôi phục lại như cũ, không cần truyền userId nữa
  const fetchResults = async () => { 
      try { 
          // Chỉ lấy tất cả kết quả, không dùng where nữa
          const q = query(collection(firestore, "exam_results"), orderBy("submittedAt", "desc")); 
          const s = await getDocs(q); 
  ...
  const handleToggleXemDapAn = async (id, currentStatus) => {
      const newStatus = !currentStatus;
      try {
          await updateDoc(doc(firestore, "quizzes", id), { choPhepXemDapAn: newStatus });
          setQuizzes(p => p.map(q => q.id === id ? { ...q, choPhepXemDapAn: newStatus } : q));
      } catch (e) { alert("Lỗi: " + e.message); }
  };
  ...
  // Bộ lọc thông minh: Lấy điểm nếu đề thi vẫn còn trong kho, HOẶC nếu đề đã xóa nhưng có lưu teacherId của thầy
  const myResults = useMemo(() => {
      return results.filter(r => {
          const isQuizExists = quizzes.some(q => q.id === r.examId);
          const isMyDeletedQuiz = r.teacherId === user?.uid;
          
          return isQuizExists || isMyDeletedQuiz;
      });
  }, [results, quizzes, user]);
  ...
  // Cập nhật lại danh sách chọn đề thi để hiển thị chữ "Đề đã xóa"
  const uniqueExams = useMemo(() => { 
      const allExamIds = [...new Set(myResults.map(r => r.examId))];
      return allExamIds.map(id => {
          const quiz = quizzes.find(q => q.id === id);
          // Tìm xem có bài nộp nào của đề này lưu tên không
          const sampleResult = myResults.find(r => r.examId === id); 
          return {
              id: id,
              title: quiz ? quiz.title : (sampleResult?.examTitle ? `${sampleResult.examTitle} (Đã xóa)` : 'Đề đã xóa (Cũ)')
          };
      });
  }, [quizzes, myResults]);
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 w-64 bg-[#020617] text-white flex flex-col border-r border-white/10 shadow-2xl z-50 transform transition-transform duration-300 ease-in-out ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        ...
        <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
           ...
           <button onClick={() => router.push('/pdf-manager')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-sm mt-1 border border-cyan-500/30 text-cyan-400 hover:bg-cyan-900/40 shadow-[0_0_15px_rgba(6,182,212,0.15)]`}><FileCheck size={18}/> ARENA ÔN THI PDF</button>
            <button onClick={() => router.push('/InitiativeManager')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl font-bold transition-all text-sm mt-1 border border-cyan-500/30 text-cyan-400 hover:bg-cyan-900/40 shadow-[0_0_15px_rgba(6,182,212,0.15)]`}><Flame size={18}/> ARENA SÁNG KIẾN</button>
...
>
    ...
    <span className="group-hover:tracking-widest transition-all">Arena Kéo Co</span>
</button>
    ...
    <div className="text-[10px] text-slate-500 flex flex-wrap gap-2 mt-1 items-center">
        <span>Mã: {q.code}</span>
        <span>• {q.questions?.length || 0} câu</span>
        <span>• {q.createdAt ? new Date(q.createdAt.seconds*1000).toLocaleDateString('vi-VN') : ''}</span>
        
        {/* HIỂN THỊ TÁC GIẢ CHỈ DÀNH CHO MASTER ADMIN */}
        {MASTER_EMAILS.includes(user?.email) && (
            <span className="text-pink-400 font-bold ml-1 bg-pink-500/10 px-1.5 py-0.5 rounded border border-pink-500/20">
                👤 TG: {q.authorEmail || q.authorName || 'Không rõ'}
            </span>
        )}
    </div>
</td>
    ...
    >
        ...
        {activeTab === 'USERS' && MASTER_EMAILS.includes(user?.email) && (
            <div className="animate-in fade-in slide-in-from-right-4 duration-500 max-w-5xl mx-auto pb-10">
                <header className="mb-8">
                    ...
                    <p className="text-slate-400 font-medium">Thống kê và quản lý người dùng toàn hệ thống</p>
                </header>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    {/* Thống kê Giáo viên */}
                    <div className="bg-gradient-to-br from-slate-900 to-[#1e293b] p-6 rounded-[2rem] border border-pink-500/30 shadow-xl relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform"><UserCog size={120} className="text-pink-500"/></div>
                        <div className="relative z-10">
                            <div className="flex items-center gap-3 mb-6">
                                <div className="bg-pink-600 p-2.5 rounded-xl shadow-lg shadow-pink-500/30"><UserCog size={24} className="text-white"/></div>
                                <h2 className="text-xl font-black uppercase text-white tracking-widest">Tài Khoản Giáo Viên</h2>
                            </div>
                            
                            <div className="grid grid-cols-3 gap-4">
                                <div className="bg-slate-800/50 p-4 rounded-2xl border border-slate-700 text-center flex flex-col items-center justify-center shadow-inner">
                                    <div className="text-3xl font-black text-white mb-1">{userStats.totalTeachers}</div>
                                    <div className="text-[10px] md:text-xs uppercase font-bold text-slate-400 tracking-wider">Tổng Số</div>
                                </div>
                                <div className="bg-green-500/10 p-4 rounded-2xl border border-green-500/30 text-center flex flex-col items-center justify-center shadow-inner">
                                    <div className="text-3xl font-black text-green-400 mb-1">{userStats.activeTeachers}</div>
                                    <div className="text-[10px] md:text-xs uppercase font-bold text-green-500 tracking-wider">Còn Hạn</div>
                                </div>
                                <div className="bg-red-500/10 p-4 rounded-2xl border border-red-500/30 text-center flex flex-col items-center justify-center shadow-inner">
                                    <div className="text-3xl font-black text-red-400 mb-1">{userStats.expiredTeachers}</div>
                                    <div className="text-[10px] md:text-xs uppercase font-bold text-red-500 tracking-wider">Hết Hạn</div>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Thống kê Học sinh */}
                    <div className="bg-gradient-to-br from-slate-900 to-[#1e293b] p-6 rounded-[2rem] border border-cyan-500/30 shadow-xl relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:scale-110 transition-transform"><GraduationCap size={120} className="text-cyan-500"/></div>
                        <div className="relative z-10 h-full flex flex-col">
                            <div className="flex items-center gap-3 mb-6">
                                <div className="bg-cyan-600 p-2.5 rounded-xl shadow-lg shadow-cyan-500/30"><GraduationCap size={24} className="text-white"/></div>
                                <h2 className="text-xl font-black uppercase text-white tracking-widest">Tài Khoản Học Sinh</h2>
                            </div>
                            
                            <div className="flex-1 flex items-center justify-center">
                                <div className="bg-cyan-500/10 w-full p-6 rounded-2xl border border-cyan-500/30 flex items-center justify-between shadow-inner">
                                    <div>
                                        <div className="text-sm uppercase font-bold text-cyan-500 tracking-wider mb-1">Tổng Số Học Sinh</div>
                                        <div className="text-xs text-slate-400">Đã tham gia hệ thống</div>
                                    </div>
                                    <div className="text-5xl font-black text-cyan-400">{userStats.totalStudents}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div className="bg-[#1e293b] p-8 md:p-12 rounded-[2rem] border border-white/10 shadow-xl text-center flex flex-col items-center justify-center">
                    ...
                    <h3 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tighter mb-4">Bảng Điều Khiển Chi Tiết</h3>
                    <p className="text-slate-400 text-sm md:text-base mb-8 max-w-xl mx-auto leading-relaxed">
                        Truy cập trang Quản Trị Hệ Thống để xem chi tiết danh sách, thêm mới, phân quyền, sửa thông tin, gia hạn tài khoản giáo viên và reset mật khẩu.
                    </p>
                    <button 
                        onClick={() => window.open('/admin/users', '_blank')}
                        className="bg-gradient-to-r from-pink-600 to-rose-600 hover:from-pink-500 hover:to-rose-500 text-white px-8 py-4 rounded-xl font-black uppercase tracking-widest shadow-lg shadow-pink-600/30 hover:shadow-pink-600/50 hover:scale-105 active:scale-95 transition-all flex items-center gap-3"
                    >
                        <Settings size={20} strokeWidth={3}/> Truy Cập Quản Lý Chi Tiết <ExternalLink size={18}/>
                    </button>
                </div>
            </div>
        )}
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <select value={userConfig.geminiModel} onChange={e=>setUserConfig({...userConfig, geminiModel: e.target.value})} className="w-full bg-slate-900 border border-slate-700 p-3 rounded-xl text-white outline-none focus:border-blue-500 text-sm">
                                        ...
                                        <option value="gemini-1.5.0-flash">Gemini 1.5 Flash (Experimental)</option>
      ...
      {selectedResultDetail && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="bg-[#0f172a] border border-cyan-500/30 shadow-[0_0_30px_rgba(6,182,212,0.15)] rounded-2xl w-full max-w-2xl overflow-hidden relative flex flex-col max-h-[90vh]">
                
                {/* Header Popup */}
                <div className="bg-slate-900 p-4 border-b border-white/10 flex justify-between items-center">
                    <div className="flex items-center gap-2">
                        <Target className="text-cyan-400" size={24} />
                        <h2 className="text-xl font-black text-white uppercase tracking-widest">Bài làm chi tiết</h2>
                    </div>
                    <button onClick={() => setSelectedResultDetail(null)} className="text-slate-400 hover:text-red-400 transition hover:scale-110 p-1">
                        <X size={24} />
                    </button>
                </div>
                ...
                <div className="p-6 overflow-y-auto flex-1 space-y-6">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5">
                            <p className="text-[10px] text-slate-400 uppercase font-bold">Chiến binh</p>
                            <p className="text-sm font-bold text-white truncate">{selectedResultDetail.studentName}</p>
                        </div>
                        <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5">
                            <p className="text-[10px] text-slate-400 uppercase font-bold">Đơn vị</p>
                            <p className="text-sm font-bold text-indigo-400">{selectedResultDetail.studentClass}</p>
                        </div>
                        <div className="bg-slate-800/50 p-3 rounded-xl border border-white/5">
                            <p className="text-[10px] text-slate-400 uppercase font-bold">Tổng điểm</p>
                            <p className={`text-xl font-black ${parseFloat(selectedResultDetail.score) >= 5 ? 'text-green-400' : 'text-red-400'}`}>{selectedResultDetail.score}</p>
                        </div>
                        <div className="bg-red-500/10 p-3 rounded-xl border border-red-500/20">
                            <p className="text-[10px] text-red-400 uppercase font-bold">Cảnh báo (Gian lận)</p>
                            <p className="text-sm font-black text-red-500">{selectedResultDetail.violations || 0} lần</p>
                        </div>
                    </div>

                    {/* Chi tiết từng câu hỏi */}
                    <div>
                        <h3 className="text-sm font-bold text-cyan-400 uppercase tracking-widest border-b border-white/5 pb-2 mb-3">Phân tích mật lệnh (Từng câu)</h3>
                        {selectedResultDetail.detail && selectedResultDetail.detail.length > 0 ? (
                            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                {selectedResultDetail.detail.map((d, idx) => (
                                    <div key={idx} className={`p-3 rounded-xl border ${d.isCorrect ? 'bg-green-500/10 border-green-500/30' : 'bg-red-500/10 border-red-500/30'} flex justify-between items-center`}>
                                        <div className="font-bold text-slate-300 text-sm">Câu {idx + 1}</div>
                                        <div className="flex flex-col items-end">
                                            <span className={`text-[10px] font-black uppercase tracking-widest ${d.isCorrect ? 'text-green-400' : 'text-red-400'}`}>
                                                {d.isCorrect ? 'Đúng' : 'Sai'}
                                            </span>
                                            <span className="text-xs font-bold text-yellow-400">+{parseFloat(d.earned || 0).toFixed(2)}đ</span>
                                        </div>
                                    </div>
                                ))}
                            </div>
                        ) : (
                            <div className="text-center py-6 bg-slate-800/30 rounded-xl border border-dashed border-white/10">
                                <p className="text-slate-500 text-sm italic">Hồ sơ cũ không có dữ liệu phân tích chi tiết.</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-slate-900 p-4 border-t border-white/10">
                    <button onClick={() => setSelectedResultDetail(null)} className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all border border-slate-600 active:scale-95">
                        Đóng Hồ Sơ
                    </button>
                </div>
            </div>
