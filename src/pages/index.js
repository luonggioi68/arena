  ...
  return (
    ...
    <div className="h-screen w-full bg-[#020617] text-white font-sans selection:bg-cyan-500 selection:text-black relative overflow-hidden flex flex-col">
      ...
      <div className="flex-1 w-full 2xl:max-w-[70%] mx-auto flex flex-col px-4 md:px-8 pt-4 pb-2 justify-between overflow-hidden">
          ...
          {/* A. NAV 1: LUYỆN TẬP */}
          <div className="shrink-0 w-full bg-black/80 backdrop-blur-xl border-2 border-red-600/50 rounded-2xl flex flex-col md:flex-row overflow-hidden shadow-[0_0_50px_rgba(220,38,38,0.4)] animate-in fade-in slide-in-from-top-4 relative z-20 h-[80px] md:h-[90px]">
                <div className="bg-gradient-to-br from-red-700 to-orange-800 p-2 md:w-48 flex flex-col items-center justify-center text-center shrink-0 relative overflow-hidden group cursor-default z-20 shadow-2xl border-b md:border-b-0 md:border-r border-red-500/30">
                    <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-30 mix-blend-overlay"></div>
                    <Target size={28} className="text-yellow-300 mb-1 animate-pulse drop-shadow-[0_0_10px_#facc15]"/>
                    <h2 className="text-lg font-black uppercase text-white leading-none tracking-tighter drop-shadow-md">Luyện Tập</h2>
                </div>

                <div className="flex-1 flex overflow-x-auto custom-scrollbar scroll-smooth z-10 relative [&::-webkit-scrollbar]:hidden">
                    <div className="flex w-full min-w-max h-full">
                        {[6, 7, 8, 9, 10, 11, 12].map((grade) => (
                            <button 
                                key={grade}
                                onClick={() => handleGradeClick(grade)}
                                className="group relative flex-1 min-w-[60px] md:min-w-0 flex flex-col items-center justify-center transition-all duration-300 border-l border-red-900/30 hover:flex-[1.3] hover:z-30 active:scale-95 overflow-hidden"
                            >
                                ...
                                <div className="relative z-10 transform group-hover:-translate-y-1 transition-transform duration-300 flex flex-col items-center">
                                    <span className="text-3xl md:text-5xl font-black italic text-transparent bg-clip-text bg-gradient-to-b from-yellow-100 to-orange-300 group-hover:from-white group-hover:to-yellow-300 transition-all duration-300 group-hover:scale-110 drop-shadow-lg">
                                        {grade}
                                    </span>
                                    <span className="text-[9px] font-bold uppercase tracking-widest text-red-300 opacity-70 group-hover:opacity-100 group-hover:text-yellow-100 absolute -bottom-3 transition-all duration-300 whitespace-nowrap">
                                        Lớp
                                    </span>
                                </div>
                            </button>
