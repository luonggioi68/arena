...
export default function HomePage() {
  ...
  const CyberCard = ({ title, subtitle, icon: Icon, color, onClick, delay }) => {
      ...
      return (
        <div 
            onClick={onClick} 
            // TĂNG MIN-HEIGHT ĐỂ THẺ ĐẦY ĐẶN HƠN (min-h-[120px])
            className={`group relative w-full h-full min-h-[120px] md:min-h-[140px] cursor-pointer rounded-2xl border border-white/5 bg-slate-900/60 backdrop-blur-sm overflow-hidden transition-all duration-300 hover:-translate-y-1 active:scale-95 flex flex-col items-center justify-center animate-in fade-in slide-in-from-bottom-8`}
        ...
        >
            ...
            <div className="relative z-10 flex flex-col items-center justify-center p-3 text-center">
                <div className={`p-3 md:p-4 rounded-xl bg-black/40 border border-white/10 mb-2 group-hover:scale-110 transition-transform duration-300 ${theme.shadow} shadow-md`}>
                    <Icon size={26} className={`md:w-8 md:h-8 ${theme.icon} drop-shadow-md`} strokeWidth={1.5} />
                </div>
                <h3 className="text-xs md:text-sm font-black italic uppercase text-white tracking-tighter leading-tight mb-1 drop-shadow-lg">
                    {title}
                </h3>
                <p className={`text-[8px] md:text-[9px] font-bold uppercase tracking-[0.1em] md:tracking-[0.2em] ${theme.text} opacity-80 group-hover:opacity-100`}>{subtitle}</p>
            </div>
        </div>
      );
  };
  ...
  return (
    <div className="min-h-screen w-full bg-[#020617] text-white font-sans selection:bg-cyan-500 selection:text-black relative overflow-hidden flex flex-col">
      {/* GLOBAL BACKGROUND CÓ THÊM ÁNH SÁNG MỜ ĐỂ BỚT TRỐNG */}
      ...
      {/* HEADER */}
      <header className="h-[60px] md:h-[70px] shrink-0 z-[100] transition-all duration-300 bg-black/40 backdrop-blur-md border-b border-white/5 shadow-lg relative">
          ...
          <div className="relative z-20 container mx-auto h-full px-3 md:px-6 flex justify-between items-center">
              <div className="flex items-center gap-2 md:gap-4">
                  {homeConfig.logoTitleImage ? (
                      <img src={homeConfig.logoTitleImage} alt="Logo" className="h-8 md:h-12 object-contain hover:scale-105 transition-transform"/>
                  ) : (
                      <div className="flex items-center gap-2 group cursor-pointer">
                        ...
                        <div className="leading-none">
                            <h1 className="text-base md:text-xl font-black italic tracking-tighter text-white uppercase drop-shadow-md">ARENA <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-purple-400">EDU</span></h1>
                            <p className="text-[7px] md:text-[8px] font-bold text-slate-300 uppercase tracking-[0.4em]">Connect</p>
                        </div>
                      </div>
                  )}
              </div>
                ...
                {user ? (
                  <div className="flex items-center gap-2 md:gap-4 bg-black/60 pl-1.5 pr-2 md:pr-6 py-1 md:py-1.5 rounded-full border border-white/10 backdrop-blur-md">
                      <div className="relative">
                          <img src={user.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.displayName || user.email)}&background=random&color=fff&size=128`} className="w-7 h-7 md:w-8 md:h-8 rounded-full border-2 border-cyan-500 object-cover" />
                          <div className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-black rounded-full hidden md:block"></div>
                      </div>
                      <div className="hidden md:block">
                          <div className="text-[8px] text-cyan-400 font-bold uppercase tracking-wider">Commander</div>
                          <div className="text-xs font-bold text-white leading-none max-w-[100px] truncate">{user.displayName || user.email}</div>
                      </div>
                      <div className="flex gap-1.5 md:gap-2 ml-1">
                          <button onClick={() => router.push('/dashboard')} className="p-1.5 bg-indigo-600 hover:bg-indigo-500 rounded-lg text-white transition"><Settings size={14} className="md:w-4 md:h-4"/></button>
                          <button onClick={() => signOut(auth)} className="p-1.5 bg-red-600/80 hover:bg-red-500 rounded-lg text-white transition"><LogOut size={14} className="md:w-4 md:h-4"/></button>
                      </div>
                  </div>
                ) : (
                  <button onClick={() => { setShowAuthModal(true); setAuthMode('LOGIN'); }} className="group relative h-8 w-8 md:h-10 md:w-10 hover:w-36 md:hover:w-44 transition-all duration-500 bg-cyan-600/90 text-white rounded-xl hover:bg-cyan-500 overflow-hidden flex items-center border border-cyan-400/30">
                      <div className="absolute inset-0 bg-white/20 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-500 skew-x-12"></div>
                      <div className="absolute left-0 w-8 h-8 md:w-10 md:h-10 flex items-center justify-center z-10 shrink-0">
                          <LogIn size={16} className="md:w-5 md:h-5 drop-shadow-md"/>
                      </div>
                      <span className="pl-8 md:pl-10 pr-2 font-black text-[9px] md:text-xs uppercase tracking-widest whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-75">
                          Đăng nhập
                      </span>
                  </button>
                )}
      ...
      {homeConfig.marqueeText && (
          <div className="w-full bg-gradient-to-r from-red-600 to-orange-600 border-b border-orange-500/50 text-white overflow-hidden flex items-center h-6 md:h-8 shrink-0 relative z-40">
              <div className="animate-marquee whitespace-nowrap font-bold text-[9px] md:text-xs tracking-widest drop-shadow-md flex items-center gap-3">
      ...
      {/* BODY CONTENT - DÀN ĐỀU CÁC THÀNH PHẦN THEO CHIỀU DỌC ĐỂ KHÔNG BỊ CỤM VÀO GIỮA */}
      <div className="flex-1 w-full lg:max-w-[80%] 2xl:max-w-[70%] mx-auto flex flex-col gap-4 md:gap-6 px-3 sm:px-4 md:px-8 py-4 overflow-y-auto overflow-x-hidden no-scrollbar">
          
          {/* MENU 8 Ô RỰC LỬA - TĂNG HEIGHT */}
          <div className="w-full grid grid-cols-4 md:grid-cols-8 gap-2 md:gap-3 shrink-0 relative z-20">
              {[...Array(8)].map((_, index) => {
                  ...
                  return (
                      <button 
                          ...
                          className="relative group min-h-[50px] md:min-h-[60px] rounded-lg md:rounded-xl font-black text-[9px] sm:text-[10px] md:text-xs uppercase tracking-wider md:tracking-widest text-white transition-all transform outline-none border-x border-t border-orange-400/80 border-b-2 md:border-b-4 border-b-red-900 shadow-lg hover:-translate-y-1 overflow-hidden p-1"
                      >
          ...
          {/* NAV LUYỆN TẬP */}
          <div className="shrink-0 w-full bg-black/80 backdrop-blur-xl border-2 border-red-600/50 rounded-xl md:rounded-2xl flex flex-col md:flex-row overflow-hidden shadow-lg animate-in fade-in slide-in-from-top-4 relative z-20">
                <div className="bg-gradient-to-br from-red-700 to-orange-800 p-3 w-full md:w-40 flex items-center justify-center text-center shrink-0 border-b border-red-500/30 md:border-b-0 md:border-r">
                    <Target size={18} className="text-yellow-300 mr-2 md:mr-0 md:mb-1 animate-pulse drop-shadow-md md:w-6 md:h-6"/>
                ...
                <div className="flex-1 flex overflow-x-auto no-scrollbar bg-slate-900/30">
                    <div className="flex w-max min-w-full md:w-full">
                        {[6, 7, 8, 9, 10, 11, 12].map((grade) => (
                            <button key={grade} onClick={() => handleGradeClick(grade)} className="group relative flex-1 min-w-[60px] md:min-w-[70px] py-3 md:py-0 md:h-[90px] flex flex-col items-center justify-center transition-all border-r border-red-900/30 last:border-0 md:border-r-0 md:border-l active:bg-red-900/40 overflow-hidden">
                                ...
                                <div className="relative z-10 flex flex-col items-center group-hover:-translate-y-0.5 transition-transform">
                                    <span className="text-2xl md:text-4xl font-black italic text-transparent bg-clip-text bg-gradient-to-b from-yellow-100 to-orange-300 group-hover:from-white group-hover:to-yellow-300 drop-shadow-md">{grade}</span>
                                    <span className="text-[8px] md:text-[10px] font-bold uppercase tracking-widest text-red-300 opacity-70 group-hover:opacity-100 group-hover:text-yellow-100 mt-0.5">Lớp</span>
                                </div>
                            </button>
                        ))}
                    </div>
          ...
          {/* GRID GAMES - CÂN ĐỐI KHOẢNG CÁCH */}
          <div className="flex-1 flex flex-col"> 
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 w-full h-full">
          ...
          {/* FOOTER BAR - TĂNG CHIỀU CAO NÚT BẤM */}
          <div className="shrink-0 w-full bg-black/80 backdrop-blur-xl border-t-2 border-cyan-600/50 rounded-xl md:rounded-2xl overflow-hidden shadow-lg relative z-20 h-[65px] md:h-[80px] mt-auto">
              <div className="flex w-full h-full bg-slate-900/50 overflow-x-auto no-scrollbar md:grid md:grid-cols-8">
                  {[...Array(8)].map((_, index) => {
                      ...
                      return (
                          <button key={index} onClick={handleClick} className={`group relative h-full flex flex-col items-center justify-center transition-colors border-r border-cyan-900/30 last:border-r-0 cursor-pointer min-w-[75px] md:min-w-0 flex-1 flex-shrink-0`}>
                              ...
                              <div className="relative z-10 flex flex-col items-center justify-center h-full w-full gap-1">
                                  {isSpin ? (
                                      <><Disc size={18} className="md:w-6 md:h-6 text-cyan-300 group-hover:rotate-180 transition-transform duration-700"/><span className="text-[9px] md:text-[10px] font-black uppercase text-cyan-100 tracking-wider text-center leading-tight">Vòng Xoay</span></>
                                  ) : isVote ? (
                                      <><BarChart2 size={18} className="md:w-6 md:h-6 text-cyan-300 group-hover:scale-110 transition-transform"/><span className="text-[9px] md:text-[10px] font-black uppercase text-cyan-100 tracking-wider text-center leading-tight">Vote</span></>
                                  ) : isPracticeBox ? (
                                      <><Target size={18} className="md:w-6 md:h-6 text-emerald-400 group-hover:scale-110 transition-transform"/><span className="text-[10px] md:text-[12px] font-black uppercase text-emerald-200 tracking-wider text-center leading-tight">Luyện Tập</span></>
                                  ) : isGrade ? (
                                      <><span className="text-xl md:text-2xl font-black italic text-transparent bg-clip-text bg-gradient-to-b from-yellow-100 to-orange-300 group-hover:text-white leading-none drop-shadow-md">{gradeNum}</span><span className="text-[8px] md:text-[9px] font-bold text-red-300 group-hover:text-yellow-100 uppercase tracking-widest leading-none">Lớp</span></>
                                  ) : null}
                              </div>
                          </button>
                      );
      ...
      {/* FOOTER INFO */}
      <footer className="h-8 shrink-0 bg-[#0a0a0a]/90 backdrop-blur border-t border-white/5 flex items-center justify-center px-4 z-20">
        <p className="text-center text-slate-600 text-[8px] md:text-[9px] font-bold uppercase tracking-widest leading-tight">
          Lượt truy cập: <span className="text-white font-mono">{realVisitorCount.toLocaleString()}</span> &nbsp;|&nbsp; © 2026 Arena Edu Connect
        </p>
      </footer>
      ...
      {/* ẨN THANH CUỘN CHO MOBILE (SWIPE MƯỢT MÀ) */}
      <style jsx global>{`
        @keyframes marquee {
