...
export default function InteractiveStudent() {
  ...
  const handleFileUpload = async (e) => {
      ...
      try {
          ...
          if (data.secure_url) setAttachment({ url: data.secure_url, type: data.resource_type, name: file.name, format: data.format });
  ...
  const handleSubmit = async () => {
      if (!content.trim() && !attachment) return alert("Viết gì đó đi chiến binh!");
      setUploading(true);
      try {
          ...
          await addDoc(collection(firestore, `interactive_boards/${id}/notes`), {
              ...
              author: name, createdAt: serverTimestamp(), approved: false, likes: 0
          });
  ...
  const handleContentClick = (e) => {
      if (e.target.tagName === 'IMG') { e.preventDefault(); e.stopPropagation(); setZoomedImage(e.target.src); }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white font-sans selection:bg-orange-500 bg-[url('https://www.transparenttextures.com/patterns/dark-matter.png')]">
      ...
      {/* HEADER */}
      <div className="h-[70px] bg-slate-950/90 border-b border-orange-500/30 px-4 flex justify-between items-center fixed top-0 w-full z-30 backdrop-blur-md shadow-lg">
          
          {/* TRÁI: LOGO + TÊN GAME (GIỮ NGUYÊN) */}
          ...
          {/* PHẢI: TÊN NHIỆM VỤ + MÃ PHÒNG + ĐỒNG HỒ + USER */}
          <div className="flex items-center gap-4">
              
              {/* [MỚI] Tên Nhiệm Vụ & Mã Phòng */}
              <div className="flex flex-col items-end mr-1">
                  <span className="text-white font-bold text-xs md:text-sm uppercase truncate max-w-[120px] md:max-w-[200px] leading-tight text-right">
                    {board?.title || "Đang tải..."}
                  </span>
                  {board?.code && (
                    <span className="flex items-center gap-1 bg-cyan-900/30 text-cyan-400 border border-cyan-500/20 px-1.5 py-0.5 rounded text-[9px] md:text-[10px] font-black font-mono tracking-widest mt-0.5">
              ...
              {/* Đồng hồ */}
              {timeLeft && (<div className={`flex items-center gap-1 md:gap-2 font-mono text-lg md:text-xl font-black ${timeLeft === "HẾT GIỜ" ? "text-red-500 animate-bounce" : "text-green-400"}`}><Timer size={18} className={timeLeft !== "HẾT GIỜ" ? "animate-pulse" : ""} /><span className="hidden md:inline">{timeLeft}</span></div>)}

              <div className="hidden md:block h-6 w-px bg-white/10"></div>

              {/* User */}
              <div className="flex items-center gap-2 bg-slate-800/80 pl-1 pr-3 py-1 rounded-full border border-white/10">
                  <div className="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold border border-white/20">{name.charAt(0)}</div>
                  <div className="hidden md:flex flex-col"><span className="text-xs text-white font-bold uppercase truncate max-w-[80px] leading-none">{name}</span><span className="text-[8px] text-slate-400 font-bold uppercase leading-none mt-0.5">{board?.status === 'OPEN' ? 'ONLINE' : 'LOCKED'}</span></div>
              </div>
          </div>
      </div>
              ...
              <div className="columns-2 md:columns-3 lg:columns-4 gap-4 space-y-4">
                  {notes.map((note) => (
                      <div key={note.id} className="break-inside-avoid bg-slate-800/60 backdrop-blur-md border border-white/10 rounded-2xl p-4 shadow-xl hover:border-orange-500/50 transition-all hover:-translate-y-1 group">
                          ...
                          <div className="text-sm text-slate-200 mb-3 break-words leading-relaxed ql-editor p-0" dangerouslySetInnerHTML={{ __html: note.content }} onClick={handleContentClick}></div>
          ...
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-end md:items-center justify-center p-0 md:p-4 animate-in fade-in duration-200">
              <div className="bg-slate-900 w-full max-w-xl md:rounded-[2rem] rounded-t-[2rem] border-t-4 md:border-4 border-orange-500 shadow-2xl p-6 relative animate-in slide-in-from-bottom duration-300">
                  ...
                  <div className="flex items-center gap-4 mb-6 bg-slate-800 p-3 rounded-xl border border-white/10">
                      ...
                      {attachment && (<div className="flex items-center gap-2 bg-slate-900 px-3 py-2 rounded-lg border border-green-500/50">{attachment.type === 'image' ? <ZoomIn size={16} className="text-green-400"/> : <FileText size={16} className="text-blue-400"/>}<span className="text-xs text-white truncate max-w-[150px]">{attachment.name}</span><button onClick={() => setAttachment(null)} className="text-red-400 hover:text-red-300"><X size={14}/></button></div>)}
                      {!attachment && !uploading && <span className="text-xs text-slate-500 italic">Ảnh, PDF, Word, Excel...</span>}
                  </div>

                  <button onClick={handleSubmit} disabled={uploading} className="w-full bg-gradient-to-r from-orange-600 to-red-600 hover:from-orange-500 hover:to-red-500 text-white font-black py-4 rounded-xl text-xl shadow-lg uppercase italic flex items-center justify-center gap-2 disabled:opacity-50 transition-all active:scale-95"><Send size={24}/> {uploading ? 'Đang xử lý...' : 'Gửi Lên Bảng'}</button>
              </div>
          </div>
      )}
