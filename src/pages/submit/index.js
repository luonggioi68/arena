...
import { UploadCloud, Link as LinkIcon, Send, CheckCircle, FileText, Loader2, User, Users, ArrowLeft, FileType, QrCode, Search, AlertCircle } from 'lucide-react';
...
export default function SubmitPage() {
  ...
  const [checking, setChecking] = useState(false); // Trạng thái đang kiểm tra mã
  const [checkResult, setCheckResult] = useState(null); // Kết quả kiểm tra: { valid: true/false, msg: '', email: '' }
  ...
  // [MỚI] HÀM KIỂM TRA MÃ NỘP BÀI
  const handleCheckCode = async () => {
      if (!formData.submissionCode.trim()) return;
      
      setChecking(true);
      setCheckResult(null); // Reset kết quả cũ

      try {
          const codeToFind = formData.submissionCode.trim().toUpperCase();
          const q = query(collection(firestore, "user_configs"), where("submissionCode", "==", codeToFind));
          const snap = await getDocs(q);

          if (snap.empty) {
              setCheckResult({ 
                  valid: false, 
                  msg: "❌ Mã không tồn tại! Vui lòng kiểm tra lại." 
              });
              ...
              if (data.email) {
                  setCheckResult({ 
                      valid: true, 
                      msg: `✅ Hợp lệ! Gửi đến: ${data.email}`,
                      email: data.email 
                  });
                  setFormData(prev => ({ ...prev, teacherEmail: data.email }));
              } else {
                  setCheckResult({ valid: false, msg: "⚠️ Mã đúng nhưng GV chưa cập nhật Email hệ thống." });
              }
          }
      } catch (error) {
          console.error(error);
          setCheckResult({ valid: false, msg: "Lỗi kết nối kiểm tra!" });
      } finally {
          setChecking(false);
      }
  };
  ...
  const handleSubmit = async (e) => {
      ...
      // Bắt buộc phải có email giáo viên (tức là đã check mã thành công)
      if (!formData.teacherEmail) {
          alert("Vui lòng bấm nút 'Kiểm tra' bên cạnh mã nộp bài trước!");
          return;
      }
  ...
  if (success) return (
      <div className="min-h-screen bg-[#0f172a] flex flex-col items-center justify-center text-white p-4 font-sans">
          ...
          <p className="text-slate-400 mb-8">Bài làm đã gửi đến giáo viên: <span className="text-cyan-400 font-bold">{formData.teacherEmail}</span></p>
          <div className="flex gap-4">
            <button onClick={() => {setSuccess(false); setFormData({...formData, content: ''}); setFileName(''); setCheckResult(null);}} className="bg-indigo-600 px-6 py-3 rounded-xl font-bold hover:bg-indigo-500 transition shadow-lg">Nộp bài khác</button>
    ...
    <div className="min-h-screen bg-[#0f172a] font-sans flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="bg-[#1e293b]/90 backdrop-blur-xl p-8 rounded-[2rem] border border-white/10 shadow-2xl w-full max-w-lg animate-in fade-in zoom-in duration-300">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            {/* [CẬP NHẬT] MÃ NỘP BÀI CÓ NÚT CHECK */}
            <div>
                <label className="block text-[10px] font-bold text-yellow-500 uppercase mb-2 ml-1">Mã nộp bài (Bắt buộc kiểm tra)</label>
                <div className="flex gap-2">
                    <div className="relative flex-1">
                        ...
                        <input 
                            ...
                            onChange={(e) => {
                                setFormData({...formData, submissionCode: e.target.value.toUpperCase(), teacherEmail: ''}); // Reset email khi sửa mã
                                setCheckResult(null); // Reset thông báo
                            }}
                    ...
                    <button 
                        ...
                        disabled={checking || !formData.submissionCode}
                        className="bg-yellow-600/20 hover:bg-yellow-600 text-yellow-500 hover:text-white border-2 border-yellow-600/50 px-4 rounded-xl font-bold transition-all disabled:opacity-50 flex items-center justify-center min-w-[50px]"
                    >
                        {checking ? <Loader2 className="animate-spin" size={20}/> : <Search size={20}/>}
                    </button>
                </div>
                
                {/* [MỚI] THÔNG BÁO KẾT QUẢ KIỂM TRA */}
                {checkResult && (
                    <div className={`mt-2 text-xs font-bold flex items-center gap-2 p-2 rounded-lg border animate-in slide-in-from-top-1 ${checkResult.valid ? 'bg-green-900/30 text-green-400 border-green-500/30' : 'bg-red-900/30 text-red-400 border-red-500/30'}`}>
                        {checkResult.valid ? <CheckCircle size={14}/> : <AlertCircle size={14}/>}
                        {checkResult.msg}
                    </div>
                )}
            </div>

            {/* CÁC PHẦN DƯỚI GIỮ NGUYÊN NHƯNG MỜ ĐI NẾU CHƯA CHECK MÃ */}
            <div className={`space-y-5 transition-opacity duration-300 ${!formData.teacherEmail ? 'opacity-50 pointer-events-none grayscale' : 'opacity-100'}`}>
            ...
            <button disabled={loading || !formData.teacherEmail} className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white py-4 rounded-xl font-black text-xl shadow-lg uppercase italic flex items-center justify-center gap-2 transition-transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed hover:shadow-[0_0_30px_rgba(79,70,229,0.5)]">
