...
import { Timer, Send, Shield, CheckCircle, User, FileText, Edit3, Eye, PenTool, ArrowLeft, Zap, Target, Trophy, Lock, Unlock, Users } from 'lucide-react';
...
export default function PDFPlay() {
    ...
    // [TÍNH NĂNG MỚI] - Mã giáo viên và Tên Lớp
        ...
        try {
            if (!studentInfo.isGuest) {
                await addDoc(collection(firestore, "exam_results"), {
                    ...
                    studentClassName: studentClassName.trim().toUpperCase() // [LƯU TÊN LỚP]
                });
            }
            setIsFinished(true);
        ...
        return (
            <div className="min-h-screen bg-[#020617] flex items-center justify-center p-4 relative overflow-hidden font-sans">
                ...
                <div className="bg-[#0f172a]/90 backdrop-blur-xl border border-cyan-500/50 p-8 md:p-10 rounded-[2rem] w-full max-w-lg shadow-[0_0_50px_rgba(6,182,212,0.3)] text-center relative z-10 animate-in zoom-in-95">
                    ...
                    {/* KHUNG NHẬP TÊN LỚP VÀ MÃ GIÁO VIÊN */}
                    <div className="bg-slate-900/80 border border-slate-700 p-4 rounded-xl mb-6 flex flex-col gap-3 text-left">
                         <div>
                             <p className="text-[10px] text-cyan-400 font-bold uppercase tracking-widest mb-1.5 flex items-center gap-1"><Users size={12}/> Tên Lớp của bạn (Tùy chọn)</p>
                             <input 
                                type="text" 
                    ...
                    <button onClick={() => setIsStarted(true)} className="w-full bg-gradient-to-r from-cyan-600 to-blue-600 text-white py-4 rounded-2xl font-black text-lg uppercase tracking-widest shadow-[0_0_30px_rgba(6,182,212,0.5)] transition-all hover:scale-105 active:scale-95">
                        BẮT ĐẦU LÀM BÀI
                    </button>
                </div>
            </div>
